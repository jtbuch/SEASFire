...
def reg_fire_freq_func(X_train_dat, y_train_dat, X_val_dat, y_val_dat, X_test_dat, lr= 1e-4, n_layers= 2, n_neurons= 16, epochs= 500, bs= 32, func_flag= 'zinb', rseed= None, samp_weights= False, samp_weight_arr= None, modsave= False):
    ...
    mdn.compile(loss=loss_func, optimizer= tf.keras.optimizers.legacy.Adam(learning_rate= lr), metrics=[acc_func])
...
def fire_size_data(res= '12km', dropcols= ['CAPE', 'Solar', 'Ant_Tmax', 'RH', 'Ant_RH'], start_month= 372, tot_test_months= 60, threshold= None, scaled= False, rseed= None, tflag= False, final_year= 2020, hyp_flag= False):
    ...
    if scaled:
        if tflag:
            clim_fire_gdf= pd.read_hdf('../data/clim_fire_size_%s_'%res + 'w%s_rescaled_data.h5'%final_year).dropna()
        ...
        if tflag:
            clim_fire_gdf= pd.read_hdf('../data/clim_fire_size_%s_'%res + 'w%s_data.h5'%final_year)
        else:
        ...
        for bs in bs_arr:
            for p_frac in pfrac_arr:
                ...
                if ml_model == 'mdn':
                    ...
                    mdn.compile(loss= zipd_loss, optimizer= tf.keras.optimizers.legacy.Adam(learning_rate= 1e-4), metrics=[zipd_accuracy])
                    ...
                    dnn.compile(loss= "binary_crossentropy", optimizer= tf.keras.optimizers.legacy.Adam(learning_rate= 1e-4), metrics= ['binary_accuracy', tf.keras.metrics.Precision(name='precision'), \
...
def fire_loc_func_obs(size_test_df, regindx, start_month, final_month= 432, ml_freq_flag= False, ml_freq_df= None, rescale= False, sf_flag= None):
    ...
    if not ml_freq_flag:
        ...
        largefreq_df= pd.read_hdf('../sav_files/large_fire_freq_scale_fac_df.h5') # scaling factor equaling the ratio of large to small fires for both observed and predicted frequencies
        ml_freq_groups= ml_freq_df.groupby('reg_indx')
        for m in np.linspace(start_month, final_month - 1, tot_months, dtype= np.int64):
            if not rescale:
...
def ml_fire_freq_cv(n_iters= 5, freq_data= 'downsampled', dropcols= ['index', 'Solar', 'Ant_Tmax', 'RH', 'Ant_RH', 'FFWI_max7', 'Avgprec_4mo', 'Avgprec_2mo', 'AvgVPD_4mo', 'AvgVPD_2mo', \
    ...
    for it in tqdm(range(n_iters)):
        ...
        if freq_data == 'downsampled':
            ...
            mdn_zipd.compile(loss= zipd_loss, optimizer= tf.keras.optimizers.legacy.Adam(learning_rate= 1e-4), metrics=[zipd_accuracy])
